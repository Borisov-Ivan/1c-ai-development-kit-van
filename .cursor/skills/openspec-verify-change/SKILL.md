...
16. **Generate Verification Report**
    ...
    **Запрещённый жаргон в Executive Summary:** `verdict: PASS/FAIL`, `verify_mode:`, `Phase A/B`, `9b`, `Blast Radius`, `precedent-regression`, `Implementation Impact Gate`, `Promotion Test`, `Determinism Test` — в пользовательский вывод не попадают (см. §3.1 `opsx-output-style.md`). Технические коды (`slice-pre`, `Standard`) допустимы только в backticks как технический код для automation.
