...
17. **Phase B — Show results + Judgment decision cards + Artifact hygiene**
    ...
    **Блок 2 — Требует вашего решения (decision-замечания, прошедшие Implementation Impact Gate):**
    ...
    **Варианты (как изменится apply):**
      a) Если выбрать <действие> → код: <…>; приёмка: <…>; риск: <…>
      b) Если выбрать <альтернатива> → код: <…>; приёмка: <…>; риск: <…>
      c) Принять как есть → код: <…>; риск: <…>
    ...
    **Блок 2b — Артефактная гигиена (`artifact-hygiene` замечания):**
    Замечания, которые не прошли Implementation Impact Gate (все три «нет»), но требуют выбора пользователя «применять или нет». Формат — однострочные пункты, **не** карточка:

    ```
    ## Артефактная гигиена (N)

    1. <Краткое описание правки и затронутые артефакты>
       Применить: [a] <вариант a — узкий scope / однозначное действие> / [b] <вариант b, если есть> / [c] отложить
    ...
    **Artifact-hygiene (Блок 2b — однострочный пункт, не карточка):**
    ...
    |---|---|
    | `scenario-uncovered` (QC 7.6) | Согласовать scope: `[a]` расширить срез / создать новый через `/opsx:extend --from-verify` / `[b]` сузить дельту spec (убрать незатронутые сценарии) / `[c]` отложить. |
    | `acceptance-scenario-duplication` (QC 5b) | Унифицировать привязку `T<M>` ↔ Scenario: `[a]` оставить в текущем срезе, удалить из источника / `[b]` оставить только в источнике / `[c]` отложить. |
    | `acceptance-without-scenario` (QC 5b) | Перенести non-scenario проверку: `[a]` `design.md#Assumptions` / `[b]` обычная `S<N>.<M>` / `[c]` `## Follow-up` / `[d]` добавить Scenario в spec и привязать `T<M>` / `[e]` отложить. |
    | `scenario-uncovered-by-acceptance` (QC 5b) | Согласовать `**Связь со spec:**`: `[a]` добавить `S<N>.T<M>` для Scenario / `[b]` снять Scenario из связи (если не закрывается здесь) / `[c]` отложить. |
    | `recipe-leaked-into-contract` (verify 7A.1) | Очистить `## Behavior Contract` от конкретных имён процедур: `[a]` пометить как «например» / `[b]` перенести в `## Implementation Options` / `[c]` отложить. |
    | `slice-numbering-inconsistent` (verify 6B) | Привести к единой нумерации в срезе: `[a]` `S<N>.<M>` через всё tasks.md / `[b]` оставить как есть / `[c]` отложить. |
    | `no-slices` (QC, legacy ЗНИ) | Миграция в срезы: `[a]` `/opsx:migrate-slices <name>` / `[b]` остаться в legacy. |
    ...
    **Self-check шага 17 (перед выводом сообщения):**
    ...
    3. Если для замечания все три строки «не меняется» — оно **не должно** быть в Блоке 2; перенести в Блок 2b (hygiene) или в «К сведению» (SUGGESTION/INFO).
    ...
    **Варианты (как изменится apply):**
      a) Зафиксировать CSV в tasks.md → код: одна процедура `ЗагрузитьCSV`; приёмка: проверка по эталонному CSV; риск: при появлении XML — отдельный срез.
      b) Зафиксировать оба формата → код: две процедуры + диспетчер; приёмка: два набора файлов; риск: трудозатраты выше.
      c) Принять как есть → код: writer выберет CSV (по умолчанию); риск: переписывание при пересмотре требования.

    Источники: QC `task-opaque`, Architect критерий 3 (Разрешённость решений).

    ## Артефактная гигиена (1)

    1. Согласовать scope дельта-spec `do2-role-autoplace`: три сценария (`throws`, `array`, `ПолныеРоли`) объявлены в дельте, но не привязаны к `**Связь со spec:**` среза S1.
       Применить: [a] узкий scope (убрать из дельты сценарии вне scope) / [b] широкий scope (добавить в Связь + регресс-задачи) / [c] отложить.
    ...
    Решений от вас: 2 (1 decision + 1 hygiene). Напишите номера выбранных вариантов (например: «1a, 2a»).
