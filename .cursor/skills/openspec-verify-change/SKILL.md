...
16. **Generate Verification Report**
    ...
    **Executive Summary (обязательная первая секция отчёта):**
    ...
    Структура **внутри** `## Executive Summary` (порядок фиксирован):

    1. **Абзац «Суть» (обязательно, первый):** только UX-слой и наблюдаемый итог проверки. Опиши **какую возможность/форму/сценарий** проверяла верификация (имена из UI или proposal в «ёлочках») и **что решено для пользователя 1С** (например: «раньше список скрывал шаги — теперь в ЗНИ явно зафиксировано, что при новой настройке шаги снова видны»). **Запрещено в этом абзаце:** пути `design.md` / `debug.md` / `tasks.md`, подстроки `## `, слова **PASS** / **FAIL**, «вердикт», «slice-gate», «QC», «Architect», название секции «Blast Radius» и любой жаргон из §3.1 `opsx-output-style.md`.
    2. **Строки метаданных проверки (обязательно, после «Сути»):**
    ...
    **Подробности:** N Блокеров, M Замечаний, K Предложений. Решений от вас: <число>.
    ...
    3. **Абзац «По документам ЗНИ» (опционально):** здесь допустимы пути к файлам, упоминание повторного прогона, что в постановке добавлено **описание, что меняется для пользователя** (не называть это именем секции движка), ссылки на архив — одним коротким абзацем. Не дублировать дословно поля YAML.
...
17. **Phase B — Show results + Judgment decision cards + Artifact hygiene**
    ...
    **Блок 2 — Решения (decision-замечания, прошедшие Implementation Impact Gate):**
    ...
    ### 1. <Заголовок без severity-метки>
    ...
    **Варианты:**
    ...
    Источники: <технический код алерта (precedent-regression-9b / dependency-cycle / slice-incomplete / task-opaque-acceptance / blast-radius-incomplete / suboptimal-architecture / …) ← QC <код> / Architect <критерий> / verify <шаг>>
    ```
    ...
    **Примеры:**
    ...
    ### 1. Неоднозначность формата в задаче 3.1

    **Что не так:** Задача 3.1 описывает «загрузку данных», но не указан формат файла (CSV / XML) и поведение при дублях ключей.

    **Что меняется для пользователя:**
    - Если оставить как есть — пользователь увидит ошибку без понятного сообщения при загрузке файла «не того» формата (writer выберет один формат по своему усмотрению).
    - Приёмка по тесту «загрузка прошла» становится субъективной — будет зависеть от того, какой именно файл взяли для проверки.
    - Если потребуется второй формат — потребуется отдельная итерация (новый срез или переделка).

    **Варианты:**
      1a — `/opsx:extend ... --from-verify` зафиксировать CSV → пользователь работает с одним эталонным форматом; при появлении XML — отдельный срез.
      1b — `/opsx:extend ... --from-verify` зафиксировать оба формата → пользователь грузит CSV и XML одной командой; трудозатраты выше.
      1c — Принять как есть → writer выбирает CSV по умолчанию; риск переписывания при пересмотре требования.

    Источники: QC `task-opaque-acceptance`, Architect критерий 3 (Разрешённость решений).

    ## Уточнения текста (1)

    2. **Согласовать scope дельта-spec `do2-role-autoplace`**: три сценария (`throws`, `array`, `ПолныеРоли`) объявлены в дельте, но не привязаны к Связи со spec в Срезе S1: «Размещение исполнителей по ролям».
       **Варианты:**
       - 2a — Применить правку (узкий scope: убрать из дельты сценарии вне фактического scope)
       - 2b — `/opsx:extend ... --from-verify` (широкий scope: добавить в Связь + регрессионные задачи)
       - 2c — Отложить
       **Не влияет на:** код Среза S1, поведение системы, приёмочные шаги S1.T1–T3.
       **Источники:** QC `scenario-uncovered`, Architect критерий 4 (Полнота покрытия).

    ## К сведению
    - 4 задачи ожидают реализации (промежуточный этап)
