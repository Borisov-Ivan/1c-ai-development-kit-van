...
Каскад (строго): снимок и хэши → детерминированная дельта → **обе метрики петли (шаг 4b, до любой карточки)** → гигиена → внешняя валидность → детерминированные связи затронутого scope → QC только изменившихся срезов → петля приёмки → трассировка Why при cache miss → профильный challenge / readiness **только** по триггеру. Дешёвый блокер (гигиена не блокирует; внешняя валидность и детерминированный FAIL) **завершает прогон** до QC и профильной эскалации. Обычный повтор: ноль или один профильный вызов. Тематический стоп и запасной счёт правок читаются в начале прохода, сразу после дельты и **до** сборки карточки нового продуктового вопроса: при срабатывании любой из метрик и неразобранной остановке продуктовая карточка в этом проходе не собирается.

```mermaid
flowchart TD
  start[verify start] --> Snap[snapshot hashes delta]
  Snap --> PatchStop{тематический стоп или запасной порог правок, остановка не разобрана?}
  PatchStop -->|да| OnlyQ[один вопрос: оставить текст или переписать срез]
  PatchStop -->|нет| L1[Layer 1: Hygiene auto-fixes]
...
|---------|----------------|------|
| Первый проход / `decision_round=0` / смена выбранного подхода или контракта поведения (хэш оси) / новый сценарий / новое продуктовое событие / ответ заказчика, меняющий правило / REJECT security·correctness / неизвестная граница дельты | `full` | все контроли заново; кэш игнорируется; независимый разбор запускается |
...
| После успешной автоматической дописки, которая только уточнила уже названную тему при том же наблюдаемом правиле | не полный независимый разбор | гигиена, продуктовая сверка, связи артефактов, опора на прошлый контроль среза; уже названную тему не поднимать как новый разрыв |
| Дополнение только фиксирует ответ заказчика внутри выбранного подхода (зеркало записанных решений и реестр контрактов изменились, ось Decisions / Behavior Contract и варианты реализации — нет) | точечный | полный независимый разбор не запускается; проверяются только затронутые темы; в отчёте видно, что прогон точечный |
| Правка выбранного подхода, контракта поведения или вариантов реализации | `full` | полный независимый разбор |
| После **user-extend** по decision (`decision_round>0`) или дельта с доказанной локальностью, кроме строчки «только ответ внутри подхода» выше | `incremental` | пересчёт `invalidation_map`; остальное из `check_cache` |
...
**Guardrails `incremental`:** после user-extend по decision, на границе среза (slice-gate) **или** когда дельта локализована картой инвалидации. После internal Repair Loop, который только уточнил уже названную тему при том же наблюдаемом правиле, следующий проход **не** назначает полный независимый разбор и **не** поднимает эту тему как новый разрыв. Полный проход остаётся для первого прохода, смены выбранного подхода или контракта поведения, нового сценария, нового продуктового события и ответа, меняющего правило. Автоматическая дописка, которая меняет наблюдаемый исход, по-прежнему даёт `verify_depth: full`. Если хэш архитектурной оси (нормализованный `design.md` § Decisions / Behavior Contract, не голый `mtime`) изменился относительно `last_challenge_at` — incremental на границе среза недоступен, нужен `full` (L4 обязателен). `/opsx:apply` на slice-gate ссылается на этот режим вместо полного прогона. Недоказанная локальность → расширить до затронутого среза; неизвестная граница → `full`.

**Та же тема (узкий повтор).** Тема, которую прошлый проход уже назвал и которая не меняет наблюдаемое правило. Уточнение формулировки при том же исходе — узкий путь: следующий проход не назначает полный независимый разбор и не называет тему новым разрывом. Смена формулы, момента второго файла, порядка шагов или иного наблюдаемого исхода — не «та же тема»: полный проход, даже без переименования заголовка.

**Стык глубины.** Первый проход полный. Смена выбранного подхода или контракта поведения — полный проход. Новый сценарий или новое продуктовое событие — полный проход. Ответ заказчика, меняющий правило, — полный проход. Открытая тема высокого авторитета не даёт лёгкий проход. Лёгкий проход при полном совпадении входов сохраняется. Точечный проход после дополнения, которое только фиксирует ответ заказчика внутри выбранного подхода, сохраняется: полный независимый разбор не запускается. Меняется одно: после автоматической дописки, которая только уточнила уже названную тему при том же наблюдаемом правиле, следующий проход не назначает полный независимый разбор. Классификация выводов разбора действует внутри разбора, когда он легитимно запущен, и не подменяет лёгкий проход.
...
### 4b. Порог правок до продуктовой карточки

Сразу после дельты, до гигиены и до любой карточки вопроса заказчику, прочитать обе метрики петли по SSOT `.cursor/rules/vertical-slices.mdc` § ДЕТЕКТОР ПЕТЛИ ПРИЁМКИ: тематическую (`TopicReopen` ≥ 2 — второе переоткрытие одной идентифицированной темы) и запасной счёт правок (`PatchRounds` против `acceptance_loop_max`, по умолчанию 3) для тем без идентичности. Момент проверки — начало прохода, не после сбора карточки.

Если сработала любая из двух метрик и остановка ещё не разобрана (нет `reports/architecture-loop-redesign-*.md` новее последней `awaiting-acceptance` этого среза и нет ответа человека «оставить текст» или «переписать срез»):

- Единственный вопрос этого прохода — «оставить текст или переписать срез».
- Продуктовая карточка в этом проходе не собирается, даже если продуктовое расхождение уже найдено: расхождение фиксируется в отчёте и в чат не выносится.
- Открытые ответы заказчика не стираются и не закрываются. Не менять `closed_decisions`, `open_decision_id`, `confirmation` и записи журнала с ответом человека из-за этой остановки.
- Дальше — карточка этого одного вопроса. Слои, которые ищут новые продуктовые развилки, в чат не выводятся.

Если остановка уже разобрана — проход идёт обычным каскадом. После ответа «оставить текст» следующий проход может задать продуктовые вопросы.
...
При любом алерте таблицы — **не** запускать новый полный независимый разбор этого прогона «из-за» блокера. Синтез отчёта и одна чат-карточка — сразу. Карточка перечисляет все открытые темы, известные к концу прогона, а не только самую раннюю: продуктовые расхождения этого прохода, остаток независимого разбора (пользовательские развилки последнего отчёта разбора, которые ещё не закрыты текстом), открытые темы журнала. Служебный указатель `open_decision_id` остаётся самым ранним открытым `EC-*` высокого авторитета и карточку не ограничивает. Формулировка «дописать вместе с ответом, не отдельным вопросом» не снимает с листа тему, уже известную к концу прогона: обе темы остаются нумерованными вопросами одной карточки. Пустой/отсутствующий реестр **и** нет события из перечня → контроль PASS без шума. Дешёвый блокер внешней валидности **всегда** раньше нового полного разбора: не запускать QC/L4/L5 «на всякий случай» параллельно с уже найденным блокером.
...
**2.1. Slice Coherence (Quality Controller)** — только **изменившиеся срезы** и связанные сценарии из `invalidation_map`. Контроль среза **не** запускать повторно, если прошлый контроль этого среза в порядке, упорядоченный набор названий сценариев среза тот же, что был на входе того контроля, и нормализованный текст обязательного пункта приёмки (Primary) не изменился. Опциональные буллеты приёмки остаются по названиям сценариев: правка формулировки внутри опционального буллета контроль не сбрасывает. В техническом аудите — отметка «опора на прошлый контроль среза» (прошлый `quality-control-*.md` взят как результат, новый полный контроль с нуля не создаётся). Детерминированные сверки покрытия (`scenario-coverage` и связанные grep) считаются как раньше и при опоре не пропускаются. Нет прошлого отчёта, прошлый контроль не в порядке, появился или исчез заголовок сценария, либо изменился текст обязательного пункта приёмки — контроль целиком. Первый прогон без прошлого контроля в порядке — полный текущий контроль (новый обязательный агентский вызов не добавляется).
...
**Метрика и порог** — SSOT `.cursor/rules/vertical-slices.mdc` § ДЕТЕКТОР ПЕТЛИ ПРИЁМКИ (тематический `TopicReopen`, запасной `PatchRounds`, `acceptance_loop_max` default 3). Обе метрики читаются на шаге 4b, в начале прохода и до карточки нового продуктового вопроса. Этот слой не переносит вопрос на конец прохода и не собирает продуктовую карточку. Если шаг 4b уже остановил проход, слой фиксирует ту же остановку и второго вопроса не добавляет. Повтор темы `EC-*` (topic-loop внешнего контракта) — **не** этот слой: см. тот же раздел «Две петли не смешивать» и § External validity.
...
3. Промпт включает:
   ...
   - **Обязательный блок Closed decisions** (из `debug.md` § Verify decision ledger + design § «Решения verify (зафиксировано)»):
     ...
     ```
   - **Леджер прошлых разрывов** (из последнего `reports/design-challenge-*.md` и журнала; если отчёта нет — пустой список). Это утверждения для перепроверки по текущему тексту, не вердикт и не запрет на новый вывод:
     ```markdown
     ## Gap ledger (recheck, not a verdict)
     - G<N> — закрыт текстом | открыт — <утверждение прошлого разбора>
     ```
     Классификация новый / повтор / сужение и наследование номера — в контракте режима `design-challenge`. Если текущий текст не подтверждает «закрыт текстом», разбор открывает тему заново.
...
- `CHALLENGE` → после **Post-challenge classifier**: если только `implementation_invariant` → Repair Loop; если `drop reopen` → не NO-GO; если `supersedes` → одна эскалация в чат; иначе **NO-GO** с одной карточкой всех открытых тем прогона (не только самой ранней). `last_challenge_at` обновляется при CHALLENGE, прошедшем в чат или repair.
...
**HARD — закрытие команды (фоновый разбор):** платформенное уведомление «фоновый разбор постановки завершён» / «отчёт сохранён» — **слот синтеза**, если карточка вердикта с **`**Следующий шаг:**`** **ещё не** ушла пользователю **в этом запуске** `/opsx:verify`. На этом ходе — **сразу** финальное сообщение по `chat-summary.md` (вариант 2 / 1a / 3a / 4), **не** служебная строка.

**Запрещено** закрывать verify фразами «дополнительных действий нет», «отчёт уже учтён», «follow-up не нужен» и аналогами **без** слота **`**Следующий шаг:**`**. Текст платформы про «no further action» **не** отменяет карточку проверки.

Если карточка **уже** была в чате этого запуска — на уведомление о фоне **молчать** (предпочтительно). Если платформа требует ответ — компактный повтор того же варианта (1a/2/3a), **не** «действий нет».

**Одна команда `/opsx:verify` → одно финальное сообщение в чат** (допустим progress marker при длинном Repair Loop). **Исключение:** канон лимита «дорогие модели недоступны — дальше на модели чата» и «Разбор постановки (Fable)» / «Разбор постановки (Opus 5)» не ждут карточки вердикта. Ход уведомления о завершении фона **до** карточки — **не** non-event и **не** «одна строка без решения» (`chat-output-budget-full.mdc` §5a). Английский progress не изобретать. Допустимые русские строки — из бюджета чата и `model-selection.mdc` (канон лимита, «Разбор постановки (Fable)», «Разбор постановки (Opus 5)», «Дописываю постановку…»).

### Repair Loop (internal, max 2 attempts)

Если шаг 4b уже остановил проход, Repair Loop продуктовую карточку не собирает: единственный вопрос — «оставить текст или переписать срез»; открытые ответы заказчика не стираются и не закрываются.
...
5. **Синтез** — оркестратор пишет **одно** финальное сообщение пользователю по `.cursor/skills/openspec-verify-change/templates/chat-summary.md` с обязательным **`**Следующий шаг:**`**. Если остановка — с вопросами заказчику и шаг 4b её не перехватил, сообщение — одна карточка всех открытых тем, известных к концу прогона (продуктовые расхождения, остаток независимого разбора, открытые темы журнала). Формат пакета — `.cursor/docs/templates/decision-block.md` § «Пакет развилок» и `templates/chat-summary.md`. Служебный `open_decision_id` (самая ранняя тема) карточку не ограничивает. Формулировка «дописать вместе с ответом, не отдельным вопросом» не снимает с листа тему, уже известную к концу прогона: обе темы остаются нумерованными вопросами одной карточки. После пакета ответов, записанных одним дополнением, следующий прогон эти темы повторно не задаёт. Промежуточные карточки Task **не цитируются** и **не упоминаются**. Уведомление о фоне без уже отправленной карточки = шаг 5, не «учтено / действий нет».
