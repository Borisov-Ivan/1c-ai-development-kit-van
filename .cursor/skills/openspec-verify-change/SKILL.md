...
3. Если **все три** флага `same/actual` **И** в текущем запросе пользователя нет содержательных вопросов (был только триггер `/opsx:verify <name>`) — путь **`silent_ok`**:
   ...
   - В чат — «тихий» вариант шаблона `templates/chat-summary.md`: **1a**, если прошлый вердикт «можно apply»; **1b**, если прошлый вердикт «до apply нужно решение». Для **1b** классифицировать блокеры из `## Что доработать в постановке` / `### Развилки` прошлого отчёта: если есть равноправная развилка для заказчика — **Вариант 3a** (один вопрос, без списка технических пунктов); если развилок нет — **Вариант 3b** (мини-абзац + автозапуск `/opsx:extend` в том же ходе, см. «Хвостовой переход»). Запрещено выводить список «Что доработать» пунктами в чате. Закончить шаг (для 3b — без END TURN, сразу extend).
...
Использовать `templates/chat-summary.md`. Первая строка — `templates/verdict-card.md`. Pre-send — `.cursor/rules/verify-user-communication.mdc`. HALT — `.cursor/rules/chat-output-budget.mdc §7`. Тон — архитектор разработчику, на языке кода 1С.

### Хвостовой переход (при NO-GO)

Если финальный вердикт NO-GO:
- Если в чате есть блок «Нужен ваш ответ» (Вариант 3a) → END TURN, ждём ответ пользователя.
- Иначе (Вариант 3b, только технические блокеры) → в том же ходе запустить скилл `/opsx:extend <name> --from-verify <отчёт>`. Дальше работает Entry Protocol extend (бриф, «Подтвердить?», запись правок).
