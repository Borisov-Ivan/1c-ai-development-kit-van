...
Старый формат `S<N>.T<M>` (несколько приёмочных задач на срез) поддерживается в legacy-режиме — verify читает оба формата, но новые ЗНИ через `/opsx:ff` генерируют только `S<N>.accept`.
...
6. `## Технический аудит (для движка OpenSpec)` — статусы слоёв, развёрнутые карточки развилок (`templates/card-decision.md`), технические алерты. **Только здесь** допустимы имена `Layer N`, `PASS/FAIL`, `CHALLENGE`, `design-challenge`, `task-readiness`.
