# Эталон «плохо»: граф есть — смысла нет

Так **не** публиковать. Манифест структурно полон (узлы, подписанные связи), но проверка смысла на шаге макета его отсекает.

Показаны **оба** провала: вывод живёт только в тексте шапки; вторая аннотация приведена без доказательства.

Отсекается пунктами self-check манифеста: `6a` (ответ на вопрос шапки не складывается с полотна) и `6d` (вторая аннотация без доказательства).
...
- Вопрос шапки («почему недостаточно удалить визуализацию») с полотна не читается: нет узла картинки штампа, нет события сброса, нет ловушки «смена макета не сбрасывает». Вывод живёт только в `header.insight` (`6a`).
- Первая аннотация с доказательством пересказывает шаг открытия, не ответ шапки. Вторая аннотация без доказательства (`6d`).
- Набор после отсева ≥4 связанных узлов — структурный порог выполнен; провал смысла порог не лечит и в текстовый резерв не уходит.
