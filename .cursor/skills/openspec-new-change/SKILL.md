...
1.5. **Metadata Gate (MANDATORY для нового change)**
   ...
   **Оговорка к инварианту:** пакет независимых продуктовых вопросов одним сообщением (шаг 5.e.2) — один акт выбора, не второй вопрос. Пошаговые гейты с зависимыми ответами (маркер, режим формы, выбор постановки, согласование срезов) остаются по одному за ход.
...
1.56. **Frontload материальных вопросов (до apply)**
   ...
   Второй пакет этот шаг **не** задаёт. Если пакет продуктовых развилок до первой проверки уже собран (шаг 5.e.2), здесь только проверка: непомеченных продуктовых развилок не осталось. Нашлась непомеченная — вернуть её в шаг 5.e.2, не задавать новый пакет.
...
5. **Create artifacts in sequence until apply-ready**
   ...
   **Error handling (MANDATORY for all Task delegations in new):**
   ...
   d.1. **Forms Mode Gate (этап design — после стабилизации scope форм, до Design Gate AskQuestion)**
      ...
      **HALT процессных преамбул:** перед вопросом режима формы **не** выводить процессные non-events («Маркер записан», «proposal/design набросаны», лог шагов new и аналоги). Сообщение в чат при **разметке / неясности** = канон вопроса из Mode Gate (и при необходимости **одна** строка, зачем нужен выбор) — без преамбулы внутренних шагов. При **только модуль** канон вопроса не показывать; допускается рекомендуемая поясняющая строка из Mode Gate (не вопрос выбора, ответа не ждать).

      **Алгоритм:**
      1. Построить список управляемых форм в scope (без Template/MXL). Пока список нестабилен — классификатор и Mode-вопросы не начинать; продолжить уточнение scope. Форму «только модуль» из списка **не** исключать.
      2. Нет форм / kit → записать `form_mode: n/a` в `## Forms mode`, вопрос **не** задавать (ветку kit не менять; не писать `bsl-only` вместо `n/a`).
      3. Для каждой формы без валидного записанного режима — **классификатор** из Mode Gate (`forms-mxl-mode-gate.mdc`) **до** канона вопроса:
         - **только модуль** → записать `bsl-only` без AskQuestion; поясняющая строка MAY (не вопрос выбора). Пустой / `n/a` на задачу модуля формы не ставить.
         - **разметка** или **неясно** → **один** вопрос этой формы (канон Mode Gate с именем **этой** формы; без процессной преамбулы — см. HALT выше) → **END TURN** → записать ответ. «Автоматически» только явным ответом. Пустой ответ на **заданный** вопрос → `manual`.
         - В **одном** ходе с записью «только модуль» (и необязательной поясняющей строкой) допускается **ровно один** вопрос из трёх по **другой** форме. Две и более формы с разметкой/неясностью — по одной за ход с паузой до ответа. Режим соседней формы не копировать.
         - Запись: `form_mode` скаляр при N=1 или map `forms:` при N≥1 / предпочтительно map при N>1.
      4. Resume: валидные режимы уже записанных форм не переспрашивать **и не перезаписывать** (логика пропуска не меняет записанный режим на `bsl-only`). Lone legacy `artifact_mode` без `form_mode`/`forms:` → считать одинаковым `form_mode` на весь текущий form-scope (не переспрашивать гомогенный набор); новая форма без записи — классификатор только для неё.
   ...
   e.2. **Пакет продуктовых развилок (после Design Gate и Slice Gate, до финальной сводки и до первой проверки):**

      1. Собрать все открытые продуктовые развилки: находки архитектора с классом «продуктовая развилка», «Открытые решения» из передачи, открытые вопросы описания (`design.md`), помеченные продуктовыми. Шаг только собирает уже помеченное; сам метку не присваивает.
      2. Продуктовая развилка — выбор наблюдаемого для заказчика поведения (что видно, когда, по какой формуле). Способ реализации в список не входит.
      3. Список пуст — молча к сводке.
      4. Список непуст — одно сообщение-пакет по `.cursor/docs/templates/decision-block.md` § «Пакет развилок»: нумерованные вопросы, у каждого два-три варианта с честным «но», одно приглашение ответить на весь пакет. **END TURN.**
      5. Ответы записываются в журнал решений (`debug.md` § `## Verify decision ledger` и зеркало в `design.md`) **до** первой проверки. Свой ответ на помеченную продуктовую развилку в постановку не вписывать.
