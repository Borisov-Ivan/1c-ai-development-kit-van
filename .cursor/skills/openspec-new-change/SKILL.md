...
- **Подтверждение постановки перед scaffold:** бриф по §5.1 (Sync Card). **B0** при свежем `## Постановка ЗНИ` в чате/handoff — **одна информирующая строка без согласования имени** (slug — техническая деталь; END TURN не делается). **B1** при свободном тексте — связный абзац «что понял / что изменится / что получите» + `Подтвердить?`, ≤6 строк; план работ в чат **запрещён**. KB-discovery — internal, в чат не выводится. `temp/briefs/*.md` не создаются.
...
1.25. **Explore / Session Context Gate**
   ...
   **Handoff Contract — поля блока `## Постановка ЗНИ` (полный список парсинга):**
   ...
   | Тема маркера | опционально | Metadata Gate (черновик domain_label) |
...
1.5. **Metadata Gate (MANDATORY для нового change)**
   ...
   **Resume (каталог `openspec/changes/<name>/` уже существует):**
   ...
   - Если в proposal плейсхолдеры — один проход как ниже (только domain_label / comment_suffix, если ФИО есть в project.md).

   **Новый change — агент собирает готовый маркер сам и предлагает принять** (выбор, а не сочинение):

   1. **Собрать черновик domain_label** (по убыванию приоритета): поле **«Тема маркера»** из блока `## Постановка ЗНИ` → выжимка из «Симптом»/«Что менять» блока → фраза из Why / What Changes. Требования: 1 фраза по-русски «что меняем и зачем», не kebab-name change, не process-метка (канон — project.md § Канон domain_label).
   2. **Если черновик собран и `defaultDeveloper` задан** — один `AskQuestion` с готовым маркером целиком:
      - Вопрос: «Маркер для кода: `// +++ <ФИО> <domain_label>` — принять?»
      - Опции: «Принять (Recommended)» / «Mechanical change — без доменного пояснения». Свой вариант пользователь даёт через Other.
   3. **Если черновик собрать не из чего** (нет блока, нет Why) — fallback: один текстовый вопрос «Доменное пояснение для маркера (1 фраза по-русски: что меняем и зачем)». Если нет и ФИО в project.md — в тот же вопрос добавить «и ФИО (формат „Фамилия И.О.“)».
   4. **Soft-reject** (только для fallback-пути и Other): ответ матчит запреты § Канон domain_label → одна строка «это process-метка, укажите доменное пояснение» и повтор (не писать в proposal).
   ...
   **Чистота гейта (HALT):** в вопрос Metadata Gate **запрещено** подмешивать другие решения (развилки A/B, выбор дизайна, имя change). Открытые развилки из постановки идут отдельной карточкой через поле «Открытые решения» (шаг 1.25).
   ...
   - Пустой suffix при новом change → только при явном выборе «Mechanical change» (`marker_style: minimal`).
