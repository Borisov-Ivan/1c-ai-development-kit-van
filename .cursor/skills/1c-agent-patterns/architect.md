...
Task(
  ...
  prompt="mode=task-readiness
         ...
         6. **Выполнимость и порядок.** Для КАЖДОЙ задачи (не только
            ...
            порядке в файле?
            - Приёмочная задача (`S<N>.accept` или legacy `S<N>.T<M>`):
              чеклист сценариев в её теле выполним пользователем
              сейчас? Все рабочие задачи `S<N>.<M>` (до приёмочной)
              завершены, чтобы пользователь мог пройти каждый буллет
              чеклиста?
