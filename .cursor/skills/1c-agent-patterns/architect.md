  ...
  prompt="mode=slice-decomposition
         ...
         10. **Self-check перед выводом `## Slices`:** для каждого среза —
            ...
            **в терминах Why/proposal**?» Если нет — объединить срезы.
            Запрещён `foundation-slice-with-gate` (QC критерий 9).
...
Task(
  ...
  prompt="mode=task-decomposition
         ...
         10.1. **Acceptance Checklist Coverage — формат `S<N>.accept`**
             ...
             - **Запрещено** создавать `S<N>.accept`, если поле `**Приёмка:**`
               среза описывает только programmatic smoke без black-box
               user-journey (QC критерий 8). Implementation Scenario →
               задача `S<N>.<M>` с пометкой «(verification, не accept)».
             - **Legacy `S<N>.T<M>`:** те же запреты — не создавать
               `S<N>.T<M>` «Проверить контракт функции …» как
               единственную приёмку среза; black-box сценарий обязателен
               (QC критерий 8, семантическая оценка).
