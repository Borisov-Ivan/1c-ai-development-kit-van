...
Task(
  ...
  prompt="mode=slice-restructuring
         ...
         11. **Remediation: non-vertical код-only срез (`slice-not-vertical`).**
            Если срез `S<K>` содержит только код (функция, хелпер) и
            приёмку «ревью контракта» / «код-ревью» / «ручная сверка»
            без наблюдаемых сценариев, а следующий срез `S<K+1>`
            **подключает** этот код — **свернуть** S<K> и S<K+1> в один
            срез: код-задачи S<K> → `S<M>.<M>` внутри объединённого
            среза; приёмка S<K+1> → `S<M>.accept` с наблюдаемым
            чеклистом; удалить блок `# Срез S<K>`; синхронизировать
            `design.md` `## Slices`. Код-ревью — внутренняя задача,
            не accept.
...
Task(
  ...
  prompt="mode=slice-decomposition
         ...
         7. Срез не может быть чисто подготовительным слоем
            ...
            срез. Подготовительный код SHALL включаться в срез с
            primary-сценарием, где код **вызывается** и даёт
            наблюдаемый результат. Срез-prereq допустим ТОЛЬКО если
            его приёмка **наблюдаема пользователем** (UI, ИБ, форма,
            трассировка). «Ревью контракта», «код-ревью», «ручная
            сверка» приёмкой среза **не считаются** — это задачи
            `S<N>.<M>`, не `S<N>.accept` / `S<N>.T<M>`. См. QC
            критерий 8 `slice-not-vertical` в
            `.cursor/rules/vertical-slices.mdc`.
         8. НЕ использовать классификацию P0–P4, «# Фаза N»,
            «<!-- phase-gate -->» — эти концепции отменены.
         9. **Acceptance Checklist Coverage** (правило среза 6 в
            `.cursor/rules/vertical-slices.mdc`): в каждом срезе будет
            **ровно одна** приёмочная задача `S<N>.accept` с буллет-
            чеклистом сценариев в теле — по одной строке на каждый
            Scenario из поля «Связь со spec» этого же среза. Инварианты
            модели, NFR / перформанс-проверки и код-уровневые ревью
            НЕ попадают в чеклист `S<N>.accept` — их место:
            - инвариант → `design.md` секция `## Assumptions`;
            - NFR / перф по коду или ТЖ → обычная задача
              `S<N>.<M>` «верифицировать по коду / ТЖ» (без accept-
              буллета);
            - ручной замер на релизе → блок `## Follow-up` в
              `tasks.md` вне срезов.
            Нельзя вкладывать «архитектурные сценарии» и «регрессы
            соседних срезов» в чеклист текущего среза — это ведёт
            к раздутой приёмке, которую пользователь не может честно
         ...
         Для каждого среза — план буллетов чеклиста, которые архитектор
         потом разместит в теле `S<N>.accept` в `tasks.md`. По одному
         буллету на каждый Scenario из поля «Связь со spec» этого же
         среза. Имена Scenario — буквально из spec.

         | Срез | S<N>.accept (бизнес-результат) | Буллеты чеклиста (Scenario) |
         |---|---|---|
         | S1 | <бизнес-результат среза> | • Scenario «<имя A>»<br>• Scenario «<имя B>» |
         | S3 | <бизнес-результат среза> | • Scenario «<имя C>» |
...
Task(
  ...
  prompt="mode=task-decomposition
         ...
         10.1. **Acceptance Checklist Coverage — формат `S<N>.accept`**
             (правило среза 6 + секция «ФОРМАТ S<N>.accept» в
             `.cursor/rules/vertical-slices.mdc`):
             - В каждом срезе **ровно одна** `S<N>.accept`. Это
               родительский чекбокс, в `[x]` которого ставится сигнал
               «срез принят целиком». Sub-bullets чеклиста **не**
               имеют отдельных чекбоксов — они описывают, что
               пользователь должен проверить, чтобы поставить `[x]`
               на родителе.
             - Тело `S<N>.accept` SHALL содержать **по одной строке**
               (sub-bullet) на каждый `#### Scenario:` из поля
               «Связь со spec» **этого** среза. Имя Scenario —
               буквально из spec, в скобках-ёлочках:
               `Scenario «<имя сценария>»: <одна строка ручного
               шага или критерия pass/fail>`.
             - Использовать «Чеклист приёмки» из секции `## Slices`
               design.md как контракт: не создавать буллеты вне этого
               списка; не создавать `S<N>.accept` без буллетов.
             - **Запрещено** в чеклисте `S<N>.accept`:
               * Scenario, заявленный в `**Связь со spec:**`
                 **другого** среза (дублирование приёмки);
               * технический инвариант модели, NFR / перформанс,
                 код-уровневое свойство без соответствующего
                 Scenario;
               * **код-ревью как приёмка:** «ревью контракта»,
                 «сверка контракта», «ручная сверка», «код-ревью» —
                 только задача `S<N>.<M>`, не accept / legacy T.
             - **Legacy `S<N>.T<M>`:** те же запреты — не создавать
               `S<N>.T<M>` «Проверить контракт функции …» как
               единственную приёмку среза; наблюдаемый сценарий в ИБ
               обязателен (QC `slice-not-vertical`).
             ...
             - **Пример OK:**
               ```
               - [ ] S3.accept Принять срез S3 — обновлённое поведение
                 автосопоставления входящих:
                 - Scenario «Несколько документов С1 без строки регистра»:
                   на тестовой ИБ прогнать
                   `КД_ДиадокСопоставлениеВходящих.ВыполнитьАвтосопоставлениеВходящих()`
                   на пакете с ≥2 С1 без строки РС — увидеть одно
                   агрегированное событие ЖР с префиксом
                   ПР.ДиадокСопоставлениеВходящих.* и перечнем
                   видов-пропусков
                 - Scenario «Один документ с зеркальной строкой РС»:
                   открыть С1 → запустить автосопоставление → строка
                   РС совпадает с эталоном
               ```
             - **BAD:** буллет «Проверить по ТЖ, что запрос к РС
               выполняется один раз на итерацию пакета.» — это NFR,
               не пользовательский сценарий. Перенести в обычную
               задачу `S1.6 Верифицировать по коду / ТЖ: один запрос
               к РегистрСведений.Диадок_ВидыДокументов на прогон
               автосопоставления (K2)`.
             - **BAD:** буллет «Подтвердить инвариант
               RecordSetModule.ПередЗаписью о наличии базовой строки.»
               — это инвариант модели данных. Перенести в
