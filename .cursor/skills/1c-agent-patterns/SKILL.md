...
## Intent-бриф (обязателен в каждом промпте Task)

Промпт субагенту описывает **цель / ограничения / scope / критерий готовности**, а не перечень микрошагов реализации.

| Поле | Содержание |
|---|---|
| **Цель** | какой исход нужен |
| **Ограничения** | guard, чего нельзя |
| **Scope** | пути и границы |
| **Критерий готовности** | когда работа сделана |

**Запрещено в промпте:** просить субагента пересказать ход внутренних рассуждений; указания «не думай долго», «думай быстрее», «skip thinking».

**Coverage-first (reviewer):** бриф требует полного покрытия проверок; приоритизация — в отчёте, не сужением охвата («только critical» в брифе запрещено).
...
**Кратко:** во frontmatter всех кастомных агентов **`model: inherit`**. Оркестратор **передаёт** `model=<slug>` из таблицы при каждом вызове (кроме ролей без Primary и второго шага цепочки — без `model=`). Лестница шагов архитектора — по таблице шагов в `.cursor/rules/model-selection.mdc` (раздел «Закрытая эскалация Fable»), без второго перечня режимов. Переопределение вне таблицы — только по явному запросу пользователя.
...
### Шаг 2: Двухшаговая цепочка

Если имя инструмента верное — цепочка из `model-selection.mdc`: шаг 2 = тот же `subagent_type` **без** `model=`. Не «Retry once» тем же вызовом и не упрощённый вариант оркестратора.

### Шаг 3: СТОП

После исчерпания цепочки — **СТОП** + канон бюджета §5. Сообщить пользователю:
...
Носитель процедуры — `review/SKILL.md` шаг 4.5. Якорь apply-reviewer и поверхности — `1c-agent-delegation.mdc` (weak / design-prescribed / agreement-override не авто-fix, не авто-waive, не AskQuestion в apply). Не пересказывать процедуру здесь.

Архитектурные замечания (новый объект, API, хранение, RLS) → СТОП, к пользователю.

---

## DELEGATION GATE

Порог и детали — `1c-agent-delegation.mdc`.

Ключевое правило: **до вызова агента допустимо ≤3 обращения к .bsl**. **Обследование** (трассировка вызовов, зависимости, сценарии, гипотезы по коду) — порог **0**. Сформулировать промпт из артефактов + пути к файлам → вызвать Task, агент сам читает код.
