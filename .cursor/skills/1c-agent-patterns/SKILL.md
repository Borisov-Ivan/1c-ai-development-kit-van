...
## FALLBACK (специализированный агент недоступен)

При ошибке вызова специализированного агента (например Invalid enum value для onec-code-writer):

1. **1 retry** — повторить вызов с тем же промптом.
2. Если retry не помог — **generalPurpose + специализированный промпт**: тот же текст задачи, плюс явная инструкция: «Ты работаешь как onec-code-writer. Прочитай `.cursor/agents/onec-code-writer.md` и следуй его правилам. Выполни задачу и верни отчёт.»
3. **Пометить в отчёте**: «Выполнено через fallback (generalPurpose) — рекомендуется повторная проверка при доступности onec-code-writer.»

### Writer (fallback через generalPurpose)

```
Task(
  description="Реализовать задачу N [feature] (fallback)",
  prompt="Ты работаешь как onec-code-writer. Прочитай .cursor/agents/onec-code-writer.md и следуй его правилам.
         ...
         Ограничение: только правка существующих .bsl, без новых файлов и метаданных.
         Следуй 1c-coding-standards.mdc. В конце отчёта укажи: «Выполнено через fallback».",
  subagent_type="generalPurpose",
  model="fast"
)
```

### Reviewer (fallback через generalPurpose)

Если onec-code-reviewer недоступен:

1. **generalPurpose** с промптом: «Прочитай `.cursor/agents/onec-code-reviewer.md`. Проведи ревью файлов [список] по всем пяти категориям (spec compliance, reuse, standards, performance, linter). Верни отчёт с замечаниями по уровням.»
2. **Дополнительно:** Grep-проверка по ключевым антипаттернам:
   - `ТекущаяДата()` в серверных модулях (должна быть ТекущаяДатаСеанса)
   - `?()` если по стандартам проекта запрещён
   - Отсутствие `#Область` в начале модуля (по std-06)
3. **Пометить в отчёте:** «Ревью через fallback, рекомендуется повторное ревью при доступности onec-code-reviewer».

```
Task(
  description="Ревью кода [feature] (fallback)",
  prompt="Прочитай .cursor/agents/onec-code-reviewer.md. Проведи ревью файлов [список изменённых .bsl] по всем пяти категориям: spec compliance, reuse, standards, performance, linter. Стандарты: 1c-coding-standards.mdc. Верни отчёт с замечаниями по уровням. В конце укажи: «Ревью через fallback».",
  subagent_type="generalPurpose",
  model="fast"
)
```
...
2. **Одна Task на пачку:** вызвать Task(generalPurpose) на оставшиеся файлы, передав образец (путь к одному изменённому файлу) + список файлов + чёткий критерий (что вставить/удалить).
