...
Используй поиск по репозиторию (Grep/SemanticSearch; если смысловой поиск недоступен — Grep и Glob, сессию не стопить) для нахождения методов БСП для регистрации и проверки имён API. Используй Glob/Read для проверки имён целевых объектов метаданных.
