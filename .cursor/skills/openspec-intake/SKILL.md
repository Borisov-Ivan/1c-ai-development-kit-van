...
7a. **Auto-handoff в рекомендованный маршрут:**
   - Маршрут однозначен → вывести "Перехожу к <маршрут>" и **в этом же ходе прочитать SKILL.md соответствующего скилла**, выполнив его Entry Protocol. Пользователь не должен вводить `/opsx:explore` / `/opsx:debug` / `/opsx:ff` / `/opsx:new` вручную. (Это легитимная смена активной команды, исключение из command-session-persistence).
   - Маршрут неоднозначен → `AskQuestion` с 2–3 вариантами маршрутов (без опции "ничего"). После выбора — переход.
   - Особый случай: при наличии трассы/стека auto-handoff в `/opsx:debug` или `/opsx:explore` с trace-brief; intake трассу не читает.
