...
# kit-van v1: -FromObject / form-add / ChildObjects registration — forbidden (Mode Gate + no-metadata)
if ($FromObject) {
	Write-Error "kit-van: -FromObject is disabled. Use -JsonPath only; create/register forms in Configurator (manual) or edit existing Form.xml via 1c-forms/edit."
	exit 1
}
if (-not $JsonPath) {
	Write-Error "-JsonPath is required."
	exit 1
}

if ($false) { # former -FromObject branch retained for upstream parity; unreachable in kit-van
...
if ($formsLeaf -eq 'Forms') {
	# kit-van v1: do NOT mutate object's ChildObjects (form-add forbidden). Form must already exist in metadata.
	Write-Host "     Skip ChildObjects registration (kit-van: form-add disabled). Ensure Form '$formName' already exists on object."
}
