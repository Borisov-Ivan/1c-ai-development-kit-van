...
- **Подтверждение постановки перед scaffold:** если пользователю нужно сверить понимание задачи — вывести адаптивный **T-BRIEF** в чате по единому каркасу §5.1 (5 слотов: **От вас / Цель / План / На выходе / Ок?**, 8–12 строк). KB-discovery (Read `openspec/knowledge/_index.yaml` и выбранные KB `.md` по путям из запроса / Explore Summary) — **внутренний шаг перед брифом**, в чат не выводится. Файлы `temp/briefs/*.md` **не создаются**.
