...
- Отменяет архивный инвариант: yes / no — <если yes: ссылка на архивный change/spec/KB/ADR и какой контракт отменяется>
...
Self-check перед выводом: слои разделены; в UX-полях нет `S<N>.T<M>` / `D<N>` / номеров задач; списки нумерованы; поле «Вход» содержит только факты; каждое поле ≤3 строк или ≤7 пунктов; поле «KB в scope» присутствует; блок **«Соответствие исходному scope»** заполнен всеми **пятью** строками (Why, Non-Goals, Behavior Contract, Отменяет архивный инвариант, Drift-check).
