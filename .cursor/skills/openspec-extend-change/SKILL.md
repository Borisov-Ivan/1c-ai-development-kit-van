...
Если классификация неоднозначна до подтверждения брифа — слот **Варианты** (brief-card B2) с русскими ярлыками, ответ **номером**; внутренний mapping → `Drift-check`:
   1. «В рамках исходной постановки» → `pass`
   2. «Расширяет постановку, но без ломки инвариантов» → `drift-warning`
   3. «Выходит за рамки / ломает ранее принятое» → `scope-violation`
   Результат отразить в брифе (agent-only коды `pass`/`drift-warning`/`scope-violation` в чат не выводить).
...
5. `debug.md` — секция `## Extend — YYYY-MM-DD`:
   ...
   - **Темы:** идентичность присваивается при первом упоминании — номер разрыва из разбора (`G<N>`) или номер контракта заказчика (`EC-*`). Повтор и сужение наследуют этот номер. Слияние тем по текстовой близости запрещено. Тема без номера в эту строку не сливается с соседней: она без идентичности и живёт на запасном счёте правок.
...
**Триггер:** user-extend с `--from-verify` после decision 3a (ответ пользователя на развилку verify, в том числе пакет ответов одной карточки).

**Пакет ответов.** Если карточка проверки содержала несколько вопросов, все ответы записываются **одной** секцией `## Extend —` и одним проходом этого шага: каждый ответ закрывает свою тему, второй заход дополнения ради остальных ответов той же карточки не требуется. После такой записи следующий прогон эти темы повторно не задаёт.
