...
**AskQuestion:**
> Выявлено N архитектурных замечаний, которые нельзя закрыть правкой кода модуля (нужны метаданные / контракты / права / точка расширения). Запустить архитектурный разбор постановки (`design.md` / RCA / openspec)? (Да / Нет — оставить в отчёте)
