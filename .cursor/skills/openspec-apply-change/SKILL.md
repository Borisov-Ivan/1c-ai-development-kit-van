...
### 6. Issue
<Развилка собирается по `.cursor/docs/templates/decision-block.md`: блок «Что решить» + триада «В чём проблема / На что влияет / Если A·B», суть inline, формулировки синтезированы по §1c (не скопированы из артефакта). Голый код решения (`OQ1`, `S1.1a`, `interim`) или непереведённая англ-метка — запрещены (§1b.8 / §1c chat-output-budget).>
