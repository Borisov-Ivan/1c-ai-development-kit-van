...
**Шаблон брифа:** адаптивный `T-BRIEF` в чате по `.cursor/docs/opsx-output-style.md` §5.1. Заголовок: `## Бриф: /opsx:debug | change: <name>`.
...
- **Развилки в чате:** если перед пользователем несколько взаимоисключающих путей (верификация гипотезы vs правка «на допущении», постановочный vs кодовый фикс и т.д.), оформить выбор блоками **«Решение N — …»** с метками `<N>a` / `<N>b` / `<N>c` по образцу `.cursor/skills/openspec-verify-change/templates/chat-summary.md`; подробности RCA остаются в `debug.md` и отчётах.
