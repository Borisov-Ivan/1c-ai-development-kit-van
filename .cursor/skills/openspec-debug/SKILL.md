...
- **Развилки в чате:** если перед пользователем несколько взаимоисключающих путей (верификация гипотезы vs правка «на допущении», постановочный vs кодовый фикс и т.д.), собрать развилку **строго** по единому контракту `.cursor/docs/templates/decision-block.md`: блок «**Что решить:** <заголовок прозой>» + триада «В чём проблема / На что влияет / Если A·B» + варианты одной строкой с «но/зато», **суть inline**. Кодов ответа (`1a`, `<N>b`) и голых кодов решения (`OQ1`, `S1.1a`) без расшифровки **не использовать** (§1b.8 `chat-output-budget.mdc`); пользователь отвечает свободным текстом. Подробности RCA остаются в `debug.md` и отчётах.
