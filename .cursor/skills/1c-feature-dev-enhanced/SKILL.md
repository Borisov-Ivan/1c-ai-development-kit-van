...
- **Верификация причин**: При багфиксах и исправлении поведения причина должна быть проверена (трасса / лог / воспроизведение) перед фазой реализации (Phase 6). Если причина — гипотеза, добавить шаг верификации в Phase 2 или между Phase 5 и Phase 6. См. правило `.cursor/rules/hypothesis-verification-gate.mdc`.

---

## Обработка исключений (Попытка/Исключение)

См. раздел «Обработка исключений» и «Когда использовать Попытка/Исключение» в `.cursor/rules/1c-coding-standards.mdc`.

**Чеклист перед добавлением обёртки Попытка/Исключение:**

1. При корректно написанном коде здесь возможна ошибка? (внешние факторы: объект удалён, свойство отсутствует в иной конфигурации, таймаут)
2. Можно ли заменить явной проверкой? (Свойство() — только для Структуры; ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта — для формы/объекта/строки; ТипЗнч(), проверка колонки, ЗначениеЗаполнено()) — если да, не использовать Попытку.
3. Если оставляем Попытку: сбой ожидаем? Лог в ЖР нужен для диагностики? Не засорять ЖР лишними записями.

**Варианты по каждому блоку:**

| Действие | Когда | Результат |
|----------|--------|-----------|
| **Убрать Попытку** | Ошибка должна быть видна (запрос, критичный ПолучитьОбъект) | Исключение всплывает |
| **Заменить явной проверкой** | Ловили вместо проверки (нет свойства, нет колонки) | Код без Попытки |
| **Оставить** | Уже есть осмысленный лог и fallback корректен | Без изменений |
| **Оставить без лога** | Сбой ожидаем (объект удалён в фоне), лог — шум | Не добавлять лог |
| **ВызватьИсключение (fail-fast)** | Структурное предусловие не выполнено (тип, свойство, размер, формат); молча пропускать нельзя | Баг виден сразу |

**Шаблоны:** ожидаемая ошибка (ПолучитьОбъект в цикле) — лог + fallback; запрос/критичная операция — убрать Попытку, пусть всплывает; проверка наличия свойства: для Структуры — Свойство("Имя"); для формы/объекта/строки ТЗ — ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта; проверку делать только там, где поле может отсутствовать.
...
5. При исследовании дефекта/бага: явно разделять находки на **Verified facts** (подтверждено кодом/трассой) и **Hypotheses** (предположения). Сохранять это разделение в артефакте phase2-exploration.md.
6. Сохранить сводку: `openspec/changes/[feature-name]/phase2-exploration.md`
...
3. Создать файл: `openspec/changes/[feature-name]/phase4-architecture.md`
   ...
   - Mermaid диаграммы
4. Если доработка затрагивает типовой код:
   - Можно ли решить через &Перед/&После? → предпочтительно
   - Нужна правка внутри метода? → &ИзменениеИКонтроль
   - Загрузить: 1c-extensions/SKILL.md
   - Зафиксировать в плане: какие методы заимствуются, какая аннотация
...
1. Запустить агента `onec-code-architect` для ревью:
   ...
   Task(
     ...
     prompt="Проверь архитектурный план для [feature].
            ...
            - Все задачи, меняющие поведение, опираются на verified facts? Если на hypotheses — есть ли задача верификации перед ними?
...
4. Если RLM доступен (сейчас NOT_CONNECTED): сохранить в RLM:
   ```
...
Когда доступен:
  ...
  Phase 8: rlm_add_hierarchical_fact(...), rlm_record_causal_decision(...), rlm_sync_state()
```
...
**Ссылка:** `.cursor/rules/1c-coding-standards.mdc` — разделы «Обработка исключений», «Когда использовать Попытка/Исключение», «Критические правила» (п. 11–13).

**Принцип:** Попытка — только там, где при корректно написанном коде может возникнуть ошибка (внешние факторы). Иначе — явные проверки, без обёртки; исключение = баг, пусть всплывает.

### Чеклист перед добавлением Попытка/Исключение

```yaml
1. При корректном коде здесь возможна ошибка?
   - Да (объект удалён в фоне, свойство в иной конфигурации отсутствует, таймаут) → можно использовать Попытку
   - Нет → не оборачивать; писать явные проверки

2. Можно заменить явной проверкой?
   - Нет свойства → Свойство("Имя") перед чтением
   - Нет колонки в выборке → проверка колонки/типа
   - Пустое значение → ЗначениеЗаполнено(), ТипЗнч()
   - Если да → убрать Попытку, добавить проверку

3. Если оставляем Попытку: нужен ли лог в Исключение?
   - Сбой ожидаем (объект удалён) и fallback корректен → лог только при необходимости диагностики
   - Не засорять ЖР «на всякий случай»
```

### Шаблоны

```yaml
# Явная проверка (вместо Попытки):
Если Объект.Свойство("Реквизит") И ЗначениеЗаполнено(Объект.Реквизит) Тогда
    Знач = Объект.Реквизит;
Иначе
    Знач = Неопределено;
КонецЕсли;

# Ожидаемый сбой (ПолучитьОбъект в цикле — мог быть удалён):
Попытка
    ДействиеОбъект = Действие.ПолучитьОбъект();
Исключение
    // осмысленный fallback, лог только если нужен для диагностики
    Возврат;
КонецПопытки;

# Не маскировать: сбой запроса/критичной операции — пусть исключение всплывает (без Попытки).
```
...
9. ✅ **Сохранение в RLM** - Phase 8: опционально (RLM NOT_CONNECTED по умолчанию)
...
#### Работа с расширениями (аннотации типового кода)
```yaml
Если в плане: "Заимствовать типовой метод" или "Изменить типовую процедуру"

Шаги:
  1. Загрузить: 1c-extensions/SKILL.md
  2. Использовать дерево решений (выбор аннотации)
  3. При &ИзменениеИКонтроль — следовать правилам формирования директив
  4. После обновления — использовать алгоритм переноса
```
