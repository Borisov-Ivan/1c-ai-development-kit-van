...
**Только при пустом вводе** (команда без текста задачи). Одна строка в чат:

«Готов разобрать задачу, дефект или вопрос. Опишите, с чем работаем — выдам короткий бриф и согласуем маршрут.»

Если пользователь уже описал задачу / дефект / вопрос — сразу Entry Protocol, bootstrap-строку не показывать.
...
**Наследник debug (дефект в открытой ЗНИ):** при профиле `explore-bug` и известном active change — RCA / решения приёмки писать в артефакты **этой** ЗНИ (`openspec/changes/<name>/debug.md` и/или `reports/`), а не только в `temp/reports/`. Правки постановки по RCA — `/opsx:extend <name>`. Explore **не** правит BSL. Вне ЗНИ — отчёты в `temp/reports/` как обычно.
...
**Наследник intake (сырой текст заказчика):** только `/opsx:explore`. До **подтверждённого** брифа запрещено читать трассу (`.pff`, `*_TRACE_*`) и код `.bsl` «чтобы разобраться» — нормализация в слоты брифа; трасса/код — после «да» через делегирование (trace-analyst / explorer).
...
По `architect-gate.mdc` / `knowledge-format.mdc` (якорь `1c-agent-delegation.mdc`). KB **не в чат-бриф**; полный список — в промпты агентов после «да».
...
**Дальше** — ровно одно: действие в Конфигураторе | `/opsx:new` / `/opsx:extend` | предложение панели (только если картинка упрощает этот ответ) | `/opsx:explain` | один вопрос | «достаточно».
```

**Приоритет слота «Дальше»:** предложение панели — только если без картинки легко перепутать ветки, условия или уровни (закрытый перечень скилла визуального объяснения). Иначе при цепочке точек — `/opsx:explain` (пошаговый разбор с кодом), одной строкой, без второго брифа. Панель и разбор в одном сообщении **запрещено**. Если печатается блок `## Постановка ЗНИ` — варианта панели нет ни в «Дальше», ни в оставшейся строке «Следующий шаг» с командой создать ЗНИ.
...
| Ответ | Действие |
|-------|----------|
| Прямая просьба («покажи схему», «покажи, как устроено», «покажи карту» и близкие) | По скиллу визуального объяснения: панель из **текущего ответа** (отчёт — если уже есть, не условие); объект-схема 1С — ответ без панели; оба чтения — одна строка, панели нет до ответа; команду сессии **не** менять |
| Согласие на предложение панели | Собрать данные из текущего ответа самому; файл пишет родитель по скиллу визуального объяснения; сборщик не вызывается; команду сессии **не** менять |
| Согласие («да», «ок», «продолжаем») на другой шаг маршрута | Следующий шаг маршрута |
| «без схемы» | Предложение панели до выхода сессии не повторять; остальной маршрут — как есть |
...
3. Есть блок «Итог / Вердикт / Дальше»? «Дальше» — конкретный шаг, не бинарное «да/нет»? Предложение панели только если картинка упрощает этот ответ, иначе при цепочке — `/opsx:explain`; оба вместе не печатать?
