...
В чат выводится компактная версия из 4 слотов:

1. **Контекст** — 1 предложение (UX-слой: что за система, процесс).
2. **Что я понял** — 1-2 предложения (суть симптома / сценария). Без внутренних ID.
3. **План** — 3-5 пунктов (агент → что исследуем).
4. **Подтвердить?** — явный вопрос "Бриф верный? Начинаем?".

#### Полный бриф (Файл)

Полная форма ВСЕГДА автоматически сохраняется в `temp/briefs/explore-<slug>-YYYY-MM-DD.md` (где `<slug>` — kebab-case темы). В чате пользователю сообщается путь к сохранённому файлу.

Содержимое полного брифа:
