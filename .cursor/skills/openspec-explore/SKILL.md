...
**Форма — связный абзац, не анкета** (лимит **8–12 строк**, escape 14 — `.cursor/rules/chat-output-budget.mdc`):

- **Абзац 2–4 предложения:** что понял из симптома/постановки (своими словами, близко к входу пользователя) → как буду искать ответ (простыми шагами, без имён агентов) → что получите на выходе (ответ в чате / блок `## Постановка ЗНИ`).
- **Вопрос** — отдельная выделенная строка: на что ищем ответ. Это **якорь** в чате; в промптах агентов передаётся как `user-goal` (внутреннее имя «Хочу понять» сохраняется для Symptom Lock и `## Постановка ЗНИ`).
- **План** — **только при реальном выборе маршрута** (2–3 пункта с осмысленными альтернативами «куда копать сначала»); рутинную последовательность шагов в чат не выносить — она остаётся в абзаце одной фразой.
- **Подтвердить?** — 1–2 строки **текстом**: «да» или уточните. **`AskQuestion` для утверждения плана не использовать**.
...
**HALT перед отправкой:** если есть `AskQuestion` для утверждения плана, или нет связного абзаца, или нет строк **Вопрос / Подтвердить?** — переписать ответ.
...
2. Есть связный абзац (что понял / как ищу / что получите) и строки **Вопрос**, **Подтвердить?**; **План** — только при реальном выборе маршрута?
