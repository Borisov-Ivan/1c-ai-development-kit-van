...
**Дальше** — ровно одно: действие в Конфигураторе | `/opsx:new` / `/opsx:extend` | `/opsx:explain` | один вопрос | «достаточно».

Если в отчёте есть **цепочка точек** (карта hardcode/настройки, call chain сценария, несколько логических гейтов) — в «Дальше» предпочтительно предложить `/opsx:explain` (пошаговый разбор с кодом), одной строкой, без второго брифа.
```
...
3. Есть блок «Итог / Вердикт / Дальше»? «Дальше» — конкретный шаг, не бинарное «да/нет»? При цепочке точек в отчёте — рассмотрен `/opsx:explain`?
