...
Если вход **слишком сырой** для брифа — нет ни симптома/цели, ни намёка на область (например «почини отчёт», «что-то с проведением», «сделай нормально») — **не выдумывать** «Что вижу / Хочу понять». Вместо этого один короткий вопрос в чат (текстом, не `AskQuestion`), закрывающий минимум для брифа:
...
- **пошагово** — `/opsx:new <name>` → `/opsx:continue <name>` (по одному артефакту, если нужно обсуждать на каждом шаге).
...
|---------|----------------|
| bug / feature | Вилка: **быстро** `/opsx:ff <name>` (берёт блок `## Для /opsx:ff` из чата — приоритет 1, handoff-файл — приоритет 2) или **пошагово** `/opsx:new <name>` → `/opsx:continue <name>`. При неудаче — спросит пользователя. |
