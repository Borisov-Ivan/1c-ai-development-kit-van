...
**Дальше** — ровно одно: действие в Конфигураторе | `/opsx:new` / `/opsx:extend` | предложение схемы | `/opsx:explain` | один вопрос | «достаточно».
```

**Приоритет слота «Дальше»:** если проверка топологии скилла карты пройдена — одно конкретное предложение схемы (что станет видно); иначе при цепочке точек — `/opsx:explain` (пошаговый разбор с кодом), одной строкой, без второго брифа. Схему и разбор в одном сообщении **запрещено**. Если печатается блок `## Постановка ЗНИ` — варианта схемы нет ни в «Дальше», ни в оставшейся строке «Следующий шаг» с командой создать ЗНИ.
...
| Ответ | Действие |
|-------|----------|
| Прямая просьба («покажи схему», «покажи карту сценария» и близкие) | По скиллу карты: однозначная карта — панель из **текущего отчёта**; однозначный объект 1С — ответ без панели; неоднозначность — одна строка, сборщика нет до ответа; команду сессии **не** менять |
| Согласие на предложение схемы | Построить панель из **текущего отчёта** исследования (путь уже есть); передать его картографу по скиллу карты; файл регистрирует родитель; команду сессии **не** менять |
| Согласие («да», «ок», «продолжаем») на другой шаг маршрута | Следующий шаг маршрута |
| «без карты» | Предложение схемы до выхода сессии не повторять; остальной маршрут — как есть |
...
3. Есть блок «Итог / Вердикт / Дальше»? «Дальше» — конкретный шаг, не бинарное «да/нет»? Сначала проверка топологии скилла карты, иначе при цепочке — `/opsx:explain`; оба вместе не печатать?
