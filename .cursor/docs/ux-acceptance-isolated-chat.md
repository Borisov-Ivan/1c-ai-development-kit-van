...
| **E4** | `/opsx:apply <name>` | нет объектов метаданных в выгрузке (WAIT Конфигуратор) | первая строка — где мы; нумерованный список «что создать»; путь **и имя раздела** файла («Что создать в Конфигураторе»); нет «0/N задач» и списка отсутствующих XML как тела; не одна ссылка на `handoff-pause-*.md`; не A/B «чеклист / имена другие» в заголовке |
...
- [ ] E4 — apply pause-wait (нет метаданных в выгрузке)
