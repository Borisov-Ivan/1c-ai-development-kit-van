---
description: Form-module code (`Form.Module.bsl` / ФормаМодуль) — client-server interaction, wiring event handlers into `Form.xml`, reserved property names forbidden as local variables, async pointers, form-data conversion.
---
> **Кейсбук kit-van (on-demand).** Канон: `.cursor/docs/casebooks/`. **Не** смешивать с `.cursor/docs/standard/std-*`. Без MCP.
