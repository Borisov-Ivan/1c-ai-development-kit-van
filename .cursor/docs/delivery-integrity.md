...
13. **Дисциплина веток:** `git log develop..main` содержит только merge-коммиты публикации и cleanup-коммиты снятия архивов/спек; содержательных прямых коммитов на `main` нет.
14. **Состав `main`:** нет каталога `openspec/` (specs/ADR/архивы живут на `develop`); есть `tools/`; заготовка шаблона ЗНИ — `.cursor/templates/seed/`.
