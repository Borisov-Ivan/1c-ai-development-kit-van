...
1. **`develop`** — линия разработки kit: specs/ADR эволюции, открытые ЗНИ и их архивы (`openspec/changes/`, `openspec/changes/archive/`). Короткую фича-ветку сразу сливать в `develop`, не оставлять домом архива.
2. **`main`** — поставка шаблона: `.cursor/**`, `AGENTS.md`, `README.md`, `tools/`. Без `openspec/` эволюции kit (specs, ADR, рабочие ЗНИ, архивы). Заготовка шаблона ЗНИ — `.cursor/templates/seed/changes/_template/`.
3. **Обновление поставки** (только из репозитория kit):
   - рабочее дерево чистое, нужные правки уже на `develop`;
   - `git checkout main`;
   - `git merge develop`;
   - удалить с `main`: каталог `openspec/` (если влился); каталог `tools/` не снимать;
   - коммит снятия архивов и спек;
   - `git push origin main` (обычный push, без `--force`);
   - в consumer-проект снова скопировать только `.cursor/` + `AGENTS.md`.
4. На `main` содержательные коммиты не делать: только merge из `develop` и cleanup-коммит снятия `openspec/`.
