...
- **Визуальное объяснение** — панель рядом с чатом: вопрос, вывод и структура текущего ответа. Открывается штатной кнопкой среды после записи файла родительским агентом. Контракт — `.cursor/skills/visual-explanation/SKILL.md`. **Не** заменяет карту точек. Ссылка в чате критерием успеха не является. Пользовательская «покажи схему» / «покажи, как устроено» без имени объекта 1С — прямая просьба этой панели, не рисунок в чате. Если среды панели нет — полный текст ответа в чате.
