...
- **Карта сценария** — панель Canvas IDE рядом с чатом: граф связей или таблица смысловых колонок механизма (эффект + доказательство). Открывается штатной кнопкой среды после записи файла родительским агентом. Контракт — `.cursor/skills/scenario-map-canvas/SKILL.md`. **Не** заменяет карту точек. Ссылка в чате критерием успеха не является. Пользовательская «покажи схему» без имени объекта 1С — прямая просьба этой панели, не текстовый резерв и не рисунок в чате.
