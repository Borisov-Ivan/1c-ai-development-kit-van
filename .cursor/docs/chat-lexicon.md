...
- **«карта точек»**, **«карточка точки»** (`/opsx:explain`) — допустимы: карта = список имён точек до прохода (**без** классов `решение` / `наблюдение` / `типовое` в чат); карточка = разбор одной точки с фрагментом кода. Не путать с success-spam («прошли все точки»).
