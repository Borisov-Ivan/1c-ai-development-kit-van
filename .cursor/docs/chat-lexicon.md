...
| `manual` (режим формы/макета) | «вручную» (правки в Конфигураторе) |
| `assisted` (режим формы/макета) | «автоматически» (правки через skill) |
| `bsl-only` (режим формы/макета) | «программно» (UX в модуле формы / коде заполнения) |
| `artifact_mode` | «режим поставки формы/макета» (код — только в proposal / mapping) |
| Mode Gate / «Mode Gate» | «вопрос режима формы/макета» (без имени гейта в прозе) |
