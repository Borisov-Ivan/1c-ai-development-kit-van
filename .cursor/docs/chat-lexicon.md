...
- **«карта точек»**, **«карточка точки»** (`/opsx:explain`) — допустимы: карта = список имён точек до прохода (**без** классов `решение` / `наблюдение` / `типовое` в чат); карточка = разбор одной точки с фрагментом кода. **Не путать** с **«картой сценария»** — панель рядом с чатом: граф связей или таблица колонок (эффект + доказательство); открывается штатной кнопкой среды; контракт — `.cursor/skills/scenario-map-canvas/SKILL.md`. Просьба «покажи схему» без уточнения объекта 1С — та же панель, не карта точек и не секция «Схема (текстовый резерв)». **Текстовый резерв** — секция журнала `explain-*.md` «Схема (текстовый резерв)», когда интерактивную панель создать нельзя; в чат не называть его картой. Не путать с success-spam («прошли все точки»). Журнал прохода в файле — не «отчёт verify»; в чат только путь.
