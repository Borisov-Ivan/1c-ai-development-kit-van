...
- **«карта точек»**, **«карточка точки»** (`/opsx:explain`) — допустимы: карта = список имён точек до прохода (**без** классов `решение` / `наблюдение` / `типовое` в чат); карточка = разбор одной точки с фрагментом кода. **Не путать** с **«картой сценария»** — панель рядом с чатом «что происходит на шаге» (эффект + доказательство); контракт — `.cursor/skills/scenario-map-canvas/SKILL.md`. Не путать с success-spam («прошли все точки»). Журнал прохода в файле (`explain-*.md`) — не «отчёт verify»; в чат только путь.
