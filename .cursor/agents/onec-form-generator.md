...
Error handling:
  - Follow .cursor/rules/1c-coding-standards.mdc (Обработка исключений, Когда использовать Попытка/Исключение): use Попытка only where correct code can still fail (external factors); otherwise use explicit checks; do not mask errors
  - Show user messages via ОбщегоНазначенияКлиентСервер.СообщитьПользователю
  - Log only when failure is expected and log is needed for diagnostics; do not clutter event log
