...
**Метка продуктовой развилки (режим постановки при создании задачи).** Открытый вопрос к заказчику о наблюдаемом поведении — что видно, когда, по какой формуле — помечать `класс: продуктовая развилка` и выносить в «Открытые решения» передачи. Оркестратор не записывает на эту развилку авторский ответ в `## Decisions`: она ждёт пакета вопросов. Вопрос о способе реализации эту метку не получает и остаётся выбором постановки.
...
- **новый** — темы не было в леджере, либо текущий текст не подтверждает статус «закрыт текстом». Такой вывод открывает тему и повтором не считается.
- **повтор** — тема уже закрыта текстом, нового подтверждённого факта нет и наблюдаемое правило не сменилось. Повтор не удерживает замечание: в дописку или в остаток при одобрении, не в блокирующий разрыв.
- **сужение** — та же тема, наблюдение уже уже прошлого вывода, правило не сменилось.

Разрывы нумеруются стабильно: первое упоминание получает номер `G<N>`, повтор и сужение наследуют его и новый номер не берут. Номер переживает прогоны. Мандат разбора не меняется: не меньше двух альтернатив и право оспорить закрытое при новом подтверждённом факте.

В отчёте обязательна дельта: какие номера закрыты текстом, какие открыты.
...
- См. `.cursor/docs/chat-lexicon.md` Слой 2: вариант = разница в коде или наблюдаемом поведении, не обход проверки (ложный apply). Иначе — в `Gaps` или удалить.
...
- **G<N>** — новый | повтор | сужение — открыт | закрыт текстом — <одна фраза>. Повтор и сужение наследуют номер первого упоминания.

## Дельта закрытого и открытого
- Закрыто текстом: <номера>
- Открыто: <номера>

## Gaps for design.md (всегда при CHALLENGE)
- **G<N>** — <конкретный пункт design / spec / proposal, который не закрыт>
...
**Когда вызывается.** Из `/opsx:verify` Layer 4 (см. `.cursor/skills/openspec-verify-change/SKILL.md`). Триггер: первый pre-apply; изменившийся хэш архитектурной оси; конфликт внешнего контракта с design; две жизнеспособные альтернативы; риск безопасности / ресурсов / прецедента. Один `mtime(design.md)` без смены хэша **не** вызывает режим — переиспользовать прошлый отчёт. Без профильного триггера режим не требуется. При неизвестной границе промпт явно расширяет scope. Анализировать только профильную дельту, затронутые оси и связанные `EC-*`; детерминированные графы Scenario↔slice не повторять. В Layer 4 архитектор обязан получить результат предыдущих слоёв (Hygiene, Internal Coherence, Problem-Solution Trace) только как **контекст**, не как замену самостоятельного аудита `proposal.md` ↔ `design.md`.

### Режим `task-readiness` — дополнительный критерий 8

Запускается из `/opsx:verify` Layer 5 **только** по профильному триггеру: изменившиеся задачи с неизвестной композицией перехватов, ручной конфигурацией, неизвестной сигнатурой/контрактом данных, новой межмодульной связью или неподтверждённым API. Без триггера режим не требуется. Scope = changed tasks + affected `EC-*`; детерминированные графы не повторять. Неизвестная граница → явно расширенный scope в промпте.
...
- **Question 1**: [описание]
  ...
  - **Blocker**: [Yes / No]
  - **Класс:** `продуктовая развилка` — только если вопрос о наблюдаемом для заказчика поведении (что видно, когда, по какой формуле). Способ реализации класс не получает. Помеченный вопрос попадает в «Открытые решения» передачи.
