...
## RELEASE-HYGIENE RULES (AP-040..AP-043)
...
2. **Release-hygiene pass (AP-040..AP-043):** использовать Intent Map (границы методов) и Contract Map (литералы, источники) — НЕ regex-скан построчно. Применить whitelist проекта. Для AP-042 прочитать tasks.md/design.md активного change (если задан).
...
Release-hygiene: K findings (AP-040..AP-043)
...
1. Для каждого finding: применить `Prerelease escalation` из AP-каталога:
...
Как детектировать `mode=prerelease`: калибровочный текст в промпте (`mode=prerelease`, вызов из `/prerelease-review`).
