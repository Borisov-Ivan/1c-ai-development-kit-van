...
Сразу после H1 по `.cursor/rules/preserve-subagent-reports.mdc` § «Шапка вводных» (таблица полей не копировать). Объект постановки — здесь, не вместо «Что наблюдаешь у себя».
...
1. **Что наблюдаешь у себя** — дословно `user-goal` из брифа (симптом пользователя, не симптом трассы; **не** список объектов из шапки `## Вводные`).
