...
**Итог исследования:** блок `## Для /opsx:ff` **в чате** (для bug-профиля). Полные отчёты `Task` — в `temp/reports/<тип>-YYYY-MM-DD-<slug>.md`. Опциональный handoff-файл `temp/explore-handoff-*.md` — только по словесной просьбе («сохрани», «зафиксируй в файл»). Каталог `openspec/sessions/` в новых исследованиях не создаётся.
