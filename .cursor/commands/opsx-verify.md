...
**Контракт для пользователя:** на вход только имя ЗНИ `<change-name>`. **Одна команда → одно финальное сообщение.** Verify **сам** чинит repair-класс (internal Repair Loop, max 2 attempts) без «Подтвердить?» и без ping-pong extend↔verify в чате. Ответ: «можно apply» | один decision-вопрос | terminal fail repair | тихий статус (1a). Chat Surface Contract — §2.6 `opsx-output-style.md`.
