...
**Контракт для пользователя:** на вход только имя ЗНИ `<change-name>` (или выводится из контекста, если ЗНИ одно). Ответ — либо короткая сводка «с прошлой проверки ничего не изменилось» (новый файл отчёта не создаётся), либо разбор: что нашли, что уже исправлено автоматически, что обычным текстом стоит обсудить. Вариантов вида «1a, 2b» нет — ответ в свободной форме.
