...
4. **Trace analysis (when trace or multi-line stack is available)**

   **4.0. Подготовить контекст для trace-analyst** (перед вызовом агента). На основе шага 2 (уже загружены proposal, design, tasks) синтезировать структурированный бриф:
   ...
   Если шаг 4.0 выполнен — передавать trace-analyst путь к трассе **и подготовленный бриф** (см. `.cursor/rules/1c-error-analysis.mdc`, шаг 1 ДЕЙСТВИЕ — «Подготовить бриф»).
   ...
   1. **Run onec-trace-analyst** (Task tool, subagent_type="onec-trace-analyst"):
      - Pass the trace file path **and the enriched context brief** (from step 4.0).
...
5. **Investigate in codebase (read-only)**

   If step 4 was already run (trace analyzed by subagents), use their output as the basis; do not duplicate manual search.
...
6. **Metadata validation (mandatory)**
...
8. **Plan and apply fixes (default: code + artifacts)**
...
- **Never invent metadata names**. Any reference to metadata or types must be validated against XML dumps in `src/` (see step 6).
- If information is insufficient: ask 1–2 targeted questions, then continue.
- Do not read traces manually — delegate to `onec-trace-analyst`. DELEGATION GATE applies.
