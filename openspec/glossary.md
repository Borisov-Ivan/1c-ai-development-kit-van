...
- **Карта сценария** — панель Canvas IDE рядом с чатом: **что происходит на шаге** (эффект + доказательство `file:line` или путь журнала). Контракт — `.cursor/skills/scenario-map-canvas/SKILL.md`. **Не** заменяет карту точек.
- **Журнал разбора** — файл `explain-*.md` по шаблону `explain-report.md`; может содержать секцию «Карта сценария», если панель недоступна.
