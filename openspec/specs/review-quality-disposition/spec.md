...
Ревьюер MUST сохранять finding по качеству, когда код слаб или рискован, даже если design/ТЗ/ADR явно допускает такое поведение. Тихий перевод в VERIFIED_OK/OK **только** из-за цитаты design MUST NOT применяться вне явно перечисленного узкого whitelist Evidence-типов (не «просто есть фраза в design»). SSOT перечня silent Evidence-типов — `.cursor/agents/onec-code-reviewer.md` § DESIGN AUTHORITY; исторический D9 — `openspec/changes/archive/2026-08-10-independent-review-disposition/design.md` § D9. Дублировать полный каталог в spec не требуется.
...
- **THEN** disposition записывается в main report (finding-id, Design ref, кто/когда), finding НЕ передаётся writer, и finding НЕ блокирует итог шага 5 / PASS отчёта (Action трактуется как waived / нормализуется оркестратором; «грязный» MUST_FIX+as-designed недопустим)
