...
Отчёты исследования по теме ЗНИ живут в каталоге задачи, а не в `temp`.
...
- **AND** если среди перенесённых есть журнал пошагового разбора — клик по фрагменту кода ведёт в `src/` с глубины каталога ЗНИ, а не остаётся ссылкой, рассчитанной на `temp/reports/`
...
- **WHEN** пользователь словами просил сохранить постановку в файл (`temp/explore-handoff-*.md` в корне `temp/`, не в `temp/reports/`) и затем создают ЗНИ по этой теме
...
- **THEN** находятся следы отчётов обследования, трассы, архитектурного разбора исследования и журнала разбора в `openspec/changes/<name>/reports/` за 7 дней, без требования, чтобы тот же файл ещё лежал в `temp`
- **AND** среди предложений нет служебных отчётов проверки постановки и приёмки соседних задач
