# Quality Control — sequential-ui-mode-questions (re-run)

**Date:** 2026-07-31  
**Mode:** slice  
**Context:** повторный QC после repair — Scenario «Empty mode blocks apply for in-scope artifact»; уточнены S2.1–S2.5 и design Decisions 6–8  
...
| S2 Раздельные режимы Form/Template | Separate form and layout delivery modes (6 scenarios) | S2.1–S2.5 | S2.accept (Primary + 3 optional; 6/6 via Primary/optional/tasks) | S1 | yes `<!-- slice-gate -->` |
...
| # | Criterion | Result |
|---|---|---|
| 1 | Scenario Coverage | Pass — все 9 `#### Scenario:` покрыты |
...
| 5b | Acceptance Checklist Coverage | Pass — Primary metadata + mandatory Primary sub-bullet в обоих срезах; empty-mode Scenario покрыт optional + tasks |
...
| 8b | Self-Achievable Acceptance | Pass — Primary S1 достижим S1.1–S1.3; Primary S2 достижим S2.1–S2.5 (в т.ч. empty-mode STOP в тех же задачах) без слоя из более позднего среза |
...
| S2.1 | Verb + file + split modes + empty STOP + (Decision) — OK |
| S2.2 | Verb + file + sequence/resume/extend + (Decision) — OK |
| S2.3–S2.4 | Verb + file + legacy/empty STOP — OK |
...
- **severity:** SUGGESTION  
- **evidence:** Spec titles are `Form-only change asks only about form`, `Layout-only change asks only about layout`, `Kit evolution without UI modes`; accept uses `«Form-only / Layout-only»` and `«Kit evolution»`. Metadata `**Связь со spec:**` abbreviates with ellipsis and omits explicit listing of Scenario «Empty mode blocks apply for in-scope artifact» (covered in accept/tasks).  
- **recommendation:** При желании выровнять имена в optional bullets и полный перечень в `**Связь со spec:**` с `#### Scenario:` буквально; на покрытие и вердикт не влияет.
...
- action: В S2.accept заменить paraphrases на literal Scenario names; в **Связь со spec:** добавить «Empty mode blocks apply for in-scope artifact» и полные имена Form-only / Layout-only / Kit evolution / Mixed / Legacy
```
...
## Delta vs previous QC (`quality-control-2026-07-31.md`)

| Item | Previous | This run |
|---|---|---|
| S2 Scenario count | 5 | 6 (+ Empty mode) |
| Empty mode coverage | absent → would be WARNING | OK (optional accept + S2.1/S2.3/S2.4) |
| S2.1–S2.5 / Decisions 6–8 | thinner | Pass — STOP empty mode, resume, extend late UI-scope explicit |
| Verdict | OK | OK |
