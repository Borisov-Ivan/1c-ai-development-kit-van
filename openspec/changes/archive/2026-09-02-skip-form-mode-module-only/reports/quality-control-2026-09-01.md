...
**Артефакты:** `tasks.md`, `design.md`, `proposal.md`, `specs/split-form-layout-modes/spec.md` (12 `#### Scenario:`)  
**debug.md:** отсутствует  
...
Mechanical pre-check (prompt, verify Layer 1): none. Чекбоксы на месте; один `<!-- slice-gate -->`; fences закрыты; `# Срез S1` + `S1.accept` с Primary; `form_mode: n/a` корректно.

Manual config checklist (verify 7.5 / Layer 5.1): маркеров ручной конфигурации этой ЗНИ нет. Подстроки «вручную» / «Конфигуратор» в задачах — токены классификатора и названия вариантов чата, не WAIT Конфигуратора.

User Task Contract pre-check (verify 2.1a): none. Grep DENY по строкам `^- \[[ x]\] S\d+\.\d+` — совпадений нет. S1.6–S1.8 — «верифицировать по тексту» (ALLOW-agent static).

Не перезаписывает `reports/quality-control-2026-09-01-slices.md` (прогон new, proposed-slice).
...
| Mixed forms sequential | `S1.accept` optional + S1.3 | OK |
| Multiple forms get sequential Mode questions | `S1.accept` optional + S1.3 | OK |
| Informing line is not a selection question | `S1.accept` optional + S1.2 | OK |
...
|---|---|---|---|
| S1.1 | Добавить в `forms-mxl-mode-gate.mdc` | классификатор «только модуль» → запись программно без выбора | длинное тело, самодостаточно; Decision 1–3, ADR-0001, имена Scenario |
| S1.2 | Добавить в `forms-mxl-mode-gate.mdc` | поясняющая строка не считается вопросом выбора | ADR-0001; Scenario informing line |
| S1.3 | Заменить в `openspec-new-change/SKILL.md` шаг 5.d.1 | классификатор до канона вопроса | ветки kit / макет / пустой ответ / соседняя форма |
...
Recipe vs outcome: S1.1/S1.3 кодируют признаки классификатора в правиле/скилле — это и есть наблюдаемый контракт kit, не частный рецепт поверх Primary.
