...
| `tools/` | утилиты разработки kit (парсер трасс; живут на `main`) | нет |
...
| Ветка | Роль |
|-------|------|
| `develop` | разработка kit: правила, навыки, specs/ADR, открытые ЗНИ, архивы |
| `main` | поставка шаблона: `.cursor/` и `AGENTS.md` (плюс `README.md` и `tools/`); без `openspec/` эволюции kit |

Правило: на `main` содержательные правки не делают. Сначала `develop`, потом публикация (слияние + снятие архивов и спек; `tools/` на `main` оставлять). Короткую фича-ветку сразу сливают в `develop`, не оставляют домом архива.

```text
правка на develop → слияние в main → снять archive/ и specs/ADR → push main
                              ↓
...
3. Удалить с `main`: `openspec/` целиком (specs, ADR, архивы ЗНИ — они остаются на `develop`). Каталог `tools/` не снимать. Заготовка шаблона ЗНИ — `.cursor/templates/seed/` (едет внутри `.cursor/`).
