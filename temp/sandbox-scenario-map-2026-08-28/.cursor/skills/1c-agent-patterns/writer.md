...
**Intent-бриф:** каждый промпт ниже содержит цель / ограничения / scope / критерий готовности. Не заменять их микрошагами «как писать код». Запрещено просить пересказать рассуждения и писать «не думай долго». Самопроверка «Попытка обоснована» (G19) — внутренний контроль агента, в Task-промпт оркестратора не включать (граница ЧТО/КАК).
...
Task(
  ...
  prompt="## Цель
         [исход задачи N для пользователя]

         ## Ограничения
         Только существующие .bsl; без новых файлов и метаданных; маркеры из proposal.

         ## Scope
         [пути .bsl]

         ## Критерий готовности
         [из tasks.md]; блоки Changes, Code-Truth Symbols, Gate Results.

         Запрещено: микрошаги реализации; просьбы пересказать рассуждения; «не думай долго».
         ...
         ## 4. INPUT CONTRACT
         [Вставить блок ## Project paths из project-paths.mdc; в kit без project.md — блок или явная пометка omit]
