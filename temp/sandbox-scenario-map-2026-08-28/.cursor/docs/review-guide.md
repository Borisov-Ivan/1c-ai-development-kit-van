...
После отчёта агент предложит устранить MUST_FIX или — если «как в постановке, но спорно» — выбрать одно из двух:

1. **Оставить как задумано** — фиксируем осознанное исключение; правка кода по этому пункту не запускается.
2. **Поставить в очередь** — пункт идёт в исправление (или в доработку постановки, если это архитектура).

«Так написано в design» само по себе не означает автоматический ок по качеству. В apply после каждой задачи агент **не** спрашивает этот выбор (скорость цикла); спорные пункты остаются со следом «потребуется подтверждение на `/review`».

Когда замечания закрыты или вы выбрали «только отчёт», а охват нетривиален — можно вызвать `/opsx:explain @…/review-*.md`: в отчёте уже есть секция `## Explain scope`, и бриф покажет обработанный код на подтверждение (для огромного release — сначала варианты рамки). Trivial light-review без findings propose не требует.
